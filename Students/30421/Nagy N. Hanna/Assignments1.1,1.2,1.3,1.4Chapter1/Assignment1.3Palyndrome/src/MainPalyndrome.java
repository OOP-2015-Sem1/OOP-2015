import java.util.*;

/**
 * 
 * @author nagyhanna007 The largest Palyndrome generated by two 3-digit nr;
 */
public class MainPalyndrome {

	public static void main(String[] args) {

		generate();
	}

	public static boolean isPalyndrome(int number) {
		String numberToString = Integer.toString(number);
		String reverse = new StringBuffer(numberToString).reverse().toString();
		if (numberToString.equals(reverse))
			return true;
		else
			return false;
	}

	public static void generate() {
		int max = 0;
		int imax = 0;
		int jmax = 0;
		// not using i >= 100 since 100*100 is not palindrome! :)
		for (int i = 9999; i > 1000; i--) {
			// because i * j === j * i, no need of both i and j
			// to count down from 999
			for (int j = i; j > 1000; j--) {
				int multi = j * i;
				if (isPalyndrome(multi) && multi > max) {
					max = i * j;
					imax = i;
					jmax = j;
				}
			}
		}

		System.out.println(
				"The largest Palyndrome generated by two 3-digit numbers " + imax + " * " + jmax + " = " + max);

	}
}