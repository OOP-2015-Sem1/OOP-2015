function  [sys, x0, str, ts] = sdspfscope2_r13(t,x,u,varargin)
...
case {2,1,3},
    ...
    user_cspec = get_pipestr_r13(pipestr,chanNumES,1);  % cspec for line lineNum
...
otherwise,
    ...
    user_cspec = get_pipestr_r13(pipestr,lineNum,1);  % cspec for line lineNum
    rgb = mapCSpecToRGB(blk, user_cspec);         % find RGB representation - empty if no match
    if nargout>1,
    	h = getColorMenuHandlesFromRGB(blk, lineNum, rgb); % get handles - may be empty
    end
...
case {2,1,3},
    ...
    lineNumMod = rem(lineNum-1, numTraces)+1;
    chanNumES = floor((lineNum-1)/numTraces)+1;
    style = get_pipestr_r13(pipestr,chanNumES,1);  % style for line lineNum
otherwise
    pipestr = get_param(blk,'LineStyles');   % get all user-specified style specs
    style = get_pipestr_r13(pipestr,lineNum,1);  % style for line lineNum
end
...
for lineNum=1:nchans,
	marker = get_pipestr_r13(pipestr, lineNum,1);
...
case {2,1,3},
    ...
    lineNumMod = rem(lineNum-1, numTraces)+1;
    chanNumES = floor((lineNum-1)/numTraces)+1;
    marker = get_pipestr_r13(pipestr,chanNumES,1);  % marker for line lineNum
otherwise
    pipestr = get_param(blk,'LineMarkers');   % get all user-specified marker specs
    marker = get_pipestr_r13(pipestr,lineNum,1);  % marker for line lineNum
end
...
disable = get_pipestr_r13(pipestr,lineNum,1);  % disable for line lineNum
...
if ~isreal(x) | isempty(x) | (floor(x) ~= x) | (x <0),
    strID = [strID '1'];
...
case 1,
    ...
    if ~isreal(y) | isempty(z) | isempty(y) | (y >= z) | (floor(y) ~= y) | (y < 0),
        strID = [strID '2'];
...
case 2,
    ...
    % -------------
    y = comm_data.offsetEye;
    if ~isreal(y) | isempty(y) | ((y-1) >= x) | (floor(y) ~= y) | (y < 0),
        strID = [strID '2'];
    	msg = ['The ''Offset'' must be a non-negative integer ' ...
...
case 1,
    ...
    if ~isreal(x) | isempty(x) | (floor(x) ~= x) | (x < 0) | (x >= 50),
        strID = [strID '3'];
...
if ~isreal(y) | isempty(x) | (floor(y) ~= y) | (y < 0),
    strID = [strID '4'];
...
if ~isreal(y) | isempty(x) | (floor(y) ~= y) | (y < 0) | (y >= comm_data.numTraces),
    strID = [strID '5'];
