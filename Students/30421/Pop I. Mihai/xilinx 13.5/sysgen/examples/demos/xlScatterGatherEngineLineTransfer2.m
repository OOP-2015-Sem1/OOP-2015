function [ arready, rvalid, rlast, rdata, rresp ] = xlScatterGatherEngineLineTransfer2( ...
    araddr, arlen, arsize, arburst,arprot, arcache, arvalid,...
    rready, resetbd, raster_scan, resetall,...
    IMAGE_COLS, IMAGE_ROWS, SWATHE_SIZE, WORD_SIZE, ...
    BANK0_START_ADDRESS, BANK1_START_ADDRESS) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
% This function behaves as two different slaves based on memory location.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Template block descriptors for row major transfers
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
persistent mem_row, mem_row = xl_state(...       
    [...
        0,  0,  0, 0, SWATHE_SIZE, IMAGE_COLS*WORD_SIZE, IMAGE_COLS*WORD_SIZE ...%Next DESC PTR, 0, START ADDRESS, 0, VSIZE, HSIZE, STRIDE, PADDING
    ],uint32);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Template block descriptors for column major transfers
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
persistent mem_col, mem_col = xl_state(...       
    [...
        0,  0,  0, 0, IMAGE_ROWS, SWATHE_SIZE*WORD_SIZE, IMAGE_COLS*WORD_SIZE ...%Next DESC PTR, 0, START ADDRESS, 0, VSIZE, HSIZE, STRIDE, PADDING
    ],uint32);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
persistent swathe_number_bank0, swathe_number_bank0 = xl_state(0, {xlUnsigned, 32, 0});
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  STATE VARAIBLE FOR STRORING LINE NUMBER - Used to define BDs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
persistent swathe_number_bank1, swathe_number_bank1 = xl_state(0, {xlUnsigned, 32, 0});
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  STATE VARAIBLE FOR STRORING BLOCK NUMBER - Used frame within a line
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
persistent addr_bank, addr_bank = xl_state(0, {xlUnsigned, 1, 0});
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
persistent next_block_descriptor_addr_bank0, next_block_descriptor_addr_bank0 = xl_state(32 + BANK0_START_ADDRESS, {xlUnsigned, 32, 0});
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  STATE VARAIBLE FOR ADDRESS of NEXT MEMORY LOCATION FOR BANK1 - Used by Scatter
%  Engine to know when to stop gathering block descriptors
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
persistent next_block_descriptor_addr_bank1, next_block_descriptor_addr_bank1 = xl_state(32 + BANK1_START_ADDRESS, {xlUnsigned, 32, 0});
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  STATE VARAIBLE FOR PERFORMING ROW WISE RASTER SCAN OF THE IMAGE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
persistent raster_scan_state, raster_scan_state = xl_state(true, {xlBoolean, 1, 0});
...
if resetbd == true,
    next_block_descriptor_addr_bank0 = 32+BANK0_START_ADDRESS;
    next_block_descriptor_addr_bank1 = 32+BANK1_START_ADDRESS;
    state = STATE_RESET;
end;
if resetall == true,
    state = RESET_ALL;
...
switch state,
    ...
    % Every thing is reinitialized
        ...
        raster_scan_state = true;
        next_block_descriptor_addr_bank0 = 32+BANK0_START_ADDRESS;
        next_block_descriptor_addr_bank1 = 32+BANK1_START_ADDRESS;  
        swathe_number_bank0 = 0;
        swathe_number_bank1 = 0;
    case STATE_RESET,
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    case STATE_ADDR_VALID_RESPONDED,
        ...
        if arlen_state == 0,
            ...
            state = STATE_RESET;
            if addr_bank == 0,
                swathe_number_bank0 = swathe_number_bank0 + 1;
            else
                swathe_number_bank1 = swathe_number_bank1 + 1;
            end;
            if addr_bank == 1,                
                next_block_descriptor_addr_bank1 = next_block_descriptor_addr_bank1 +  32;
            else                            
                next_block_descriptor_addr_bank0 = next_block_descriptor_addr_bank0 +  32;                            
            end;
        else
            rvalid_state = true;                   
            if (arlen_state ~= 0)                                    
                if mem_addr == 2, % START_ADDR - MAC for address computation
                    ...
                    % performance 
                    if addr_bank == 0,
                        if raster_scan_state == true,
                            rdata_state = BYTES_PER_SWATH*swathe_number_bank0;% + 67108868;
                        else
                            rdata_state = WORD_SIZE*swathe_number_bank0;% + 67108868;
                        end;
                    else
                        if raster_scan_state == true,
                            rdata_state = BYTES_PER_SWATH*swathe_number_bank1;% + 67108868;
                        else
                            rdata_state = WORD_SIZE*swathe_number_bank1;% + 67108868;
                        end;                            
                    end;
                elseif mem_addr == 0,
                    if addr_bank == 1,
                        rdata_state = next_block_descriptor_addr_bank1;                        
                    else
                        rdata_state = next_block_descriptor_addr_bank0;                        
                    end;
                else
                    if raster_scan_state == true,
                        rdata_state = mem_row(mem_addr);                                            
                    else
                        rdata_state = mem_col(mem_addr);                                            
                    end;
                end;
                if arlen_state == 1,                        
                    rlast_state = true;
