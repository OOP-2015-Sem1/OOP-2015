...
function  WWHControls_Initialize()
{
  ...
  // -sps- always enable pdf -
  this.fAddControl("WWHPDFIcon", VarSettings.mbPDFEnabled, true,
...
function  WWHControls_LeftHTML()
{
  ...
  if (this.fGetControl("WWHFrameSetIcon") != null)
  {
    ...
    // -sps ICE bug of not printing background if nothing printed
    if (WWHFrame.WWHBrowser.mbICEBrowser) {
	VarHTML += "<br><br>";
    }
...
function  WWHControls_RightHTML()
{
  ...
  if (this.fGetControl("WWHRelatedTopicsIcon") != null)
  {
    ...
    // - sps - ICE bug of not displaying background if nothing printed.
    if (WWHFrame.WWHBrowser.mbICEBrowser) {
	VarHTML += "<br><br>";
    }
...
function  WWHControls_PDF()
{
  ...
  //}
  // - sps - just open the user_guide
  //PDFLink=WWHFrame.WWHHelp.mBaseURL+"/"+WWHBrowser.mbUserGuidePDF;

  //top.document.location.href=PDFLink;
WWHFrame.WWHBrowser.fMLProtocol("open(fullfile(xilinx.environment.getpath('sysgen'),'"+WWHBrowser.mbUserGuidePDF+"'));");
