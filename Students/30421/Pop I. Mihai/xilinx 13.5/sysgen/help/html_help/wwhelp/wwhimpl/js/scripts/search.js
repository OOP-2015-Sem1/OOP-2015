...
function  WWHSearch_DisplaySearchForm()
{
  ...
  var  Level;
    ...
    // ICE browser has problems with 
    if (WWHFrame.WWHBrowser.mbICEBrowser) {
	HTML += "<form name=\"WWHSearchForm\" onSubmit=\"WWHFrame.WWHSearch.fSubmit();return false;\">\n";
	HTML += "<nobr>\n";
	...
	HTML += "<input type=\"button\" value=\"" + WWHFrame.WWHJavaScript.mMessages.mSearchButtonLabel;
	HTML += "\" onClick=\"WWHFrame.WWHSearch.fSubmit();\">\n";
    } else {
      ...
      (this.mSearchState == null))
  {
    ...
    if (this.mSearchState == "words")
    {
      if (this.mSavedSearchWords.length > 0)
      {
        ...
        //
	 // - sps - If using ICE browser, we don't use the submit methodology because
	 // it doesn't work like other browser, so we use the onClick, which requires a reload
	 if (WWHFrame.WWHBrowser.mbICEBrowser) {
	    WWHFrame.WWHJavaScript.mPanels.fReloadView();
	}
      }
