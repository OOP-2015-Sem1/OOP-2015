...
#  /___/  \  /   Vendor: Xilinx
#  \   \   \/    Version: 2.0
#   \   \        Filename: fir_compiler_v3_0_utils.tcl
...
namespace eval fir_compiler_v3_0_utils {
  ...
  namespace export log2ceil log2int num_BRAM
   ...
   proc Num_DSPs { c_xdevice } {
     ...
     return [ expr { ( [ fir_compiler_v3_0_utils::Num_Full_DSP_Col $c_xdevice ]               \
                   *   [ fir_compiler_v3_0_utils::Num_DSP_per_Full_Col $c_xdevice ] )         \
                   + ( [ fir_compiler_v3_0_utils::Num_Half_DSP_Col $c_xdevice ]               \
                   * ( [ fir_compiler_v3_0_utils::Num_DSP_per_Half_Col $c_xdevice ] / 2 ) ) } \
             ]
   ...
   proc log2int { value } {
     ...
     set abc1 [ fir_compiler_v3_0_utils::log2 $value ]
   ...
   proc get_data_mem_type { mem_option data_mem_space num_channels srl16_depth_thres coef_struct filt_type rate_type data_buff_type } {
     ...
     if { $mem_option == "Automatic" } {
       ...
       #set temp_val [ expr { $num_channels * pow(2, [fir_compiler_v3_0_utils::log2ceil  $data_mem_space  ]) } ]
   ...
   proc get_coef_mem_type { mem_option coef_mem_space num_filts coef_reload dram_depth_thres srl16_depth_thres coef_buff_type } {
     ...
     if { $mem_option == "Automatic" } {
       ...
       #set temp_val [ expr { $num_filts * pow(2, [ fir_compiler_v3_0_utils::log2ceil  $coef_mem_space  ]) } ]
       set temp_val [ expr { $num_filts * $coef_mem_space } ]
       set cmp_depth [ expr { pow(2,[ fir_compiler_v3_0_utils::log2ceil  $temp_val   ] ) } ]
       if { $coef_reload == true } {
   ...
   ############################################################################
   proc get_bit_growth { filt_arch filt_type coef_struct num_taps inter_rate } {
     ...
     ;# puts "Calculated log2 of $num_taps as $bit_growth"
   ...
   ############################################################################
   proc get_output_width { filt_arch bit_growth coef_sign coef_width data_sign data_width } {
     ...
     if { $local_data_width == 1 && $local_coef_width == 1 } {
       if { $data_sign == "Signed" && $coef_sign == "Signed" } {
         set output_width [ expr { 2 + $bit_growth } ]
       } else {
         set output_width $bit_growth
       }
     } elseif { $local_data_width == 1 } {
       set output_width [ expr { $local_coef_width + $bit_growth } ]
     } elseif { ( $local_coef_width == 1 ) } {
       set output_width [ expr { $local_data_width + $bit_growth } ]
     } else {
       set output_width [ expr { $local_data_width + $local_coef_width + $bit_growth } ]
     }
     return $output_width
   }
