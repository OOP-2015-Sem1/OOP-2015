...
public class Handler implements MouseListener {
	...
	public void mouseClicked(MouseEvent e) {
		...
		//this.gameFor2(((PlayerCardsPanel) ((Card) e.getSource()).getParent()), ((Card) e.getSource()));
	...
	public void draw2Cards(PlayerCardsPanel panel, Player player) {
		player.getHand()
				.add(Game.gameWindow.deckPanel.getDeck().get(Game.gameWindow.deckPanel.getDeck().size() - 1));
			...
			for(Player player: Game.players){
				if(player.isTurn()){
					...
					if (card.isSpecial) {
						...
						if (((SpecialCard)card).isWild()&&(((SpecialCard)card).getDraw() == Card.DRAW_FOUR)){
							...
							this.gameSecurity(Game.gameWindow.turn+Game.gameWindow.reverse);
							this.draw2Cards(Game.players.get(Game.gameWindow.turn).playerCardsPanel, Game.players.get(Game.gameWindow.turn));
							this.draw2Cards(Game.players.get(Game.gameWindow.turn).playerCardsPanel, Game.players.get(Game.gameWindow.turn));
							this.gameSecurity(Game.gameWindow.turn+Game.gameWindow.reverse);
							Game.players.get(Game.gameWindow.turn).setTurn(true);
				...
				if(Game.players.get(Game.gameWindow.turn).isTurn()){
					if (card.isSpecial == false) {
						if (card.getColor()
								.equals(Game.gameWindow.gameColor)
						...
						if (((SpecialCard) card).isReverse()
								&& ((SpecialCard) card).getColor()
								.equals(Game.gameWindow.gameColor)
						|| (((SpecialCard) card).isReverse() && ((SpecialCard) Game.gameWindow.releasedCards.cards
