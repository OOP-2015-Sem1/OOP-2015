...
import game.HandValue;  
  ...
  class Interface extends JPanel {  
	...
	private int bet;
	private String text;
	private String path="D:\\eclipse\\BlackJack\\src\\";
   ...
   Interface(){  
	 ...
	 setLayout(null);	  
	   JButton hit=new JButton("HIT");  
	    ...
	    JLabel label1 = new JLabel("DOUBLE");
	    JLabel label2 = new JLabel("DOWN");
	    dd.add(BorderLayout.CENTER,label1);
	    dd.add(BorderLayout.SOUTH,label2);
	    JButton pa = new JButton();
	    pa.setLayout(new BorderLayout());
	    JLabel label3 = new JLabel("PLAY");
	    JLabel label4 = new JLabel("AGAIN");
	    pa.add(BorderLayout.CENTER,label3);
	    pa.add(BorderLayout.SOUTH,label4);
...
void drawCard(Graphics g, Card card, int x, int y) {
	...
	if (card==null)
	{ try {image=ImageIO.read(new File(path+"back.jpg"));
	  } catch (IOException e) {
	}
	 ...
	 try {
		 image = ImageIO.read(new File(path+card.getValueAsString()+"_of_"+card.getSuitAsString()+".png"));
		} catch (IOException e) {
		}
	 if(image!=null){
			g.drawImage(image, x, y, 100,120,null);
	...
	void newBet(){
		...
		do { 
			betString=JOptionPane.showInputDialog("You have "+yourMoney+". How much you want to bet?");
				...
				if (betString==null)System.exit(0);
				try{
	...
	private  void pressHit(){
		if (gameStatus==false){text = "Click \"Play Again\" to play again.";
			repaint();return;}
	    ...
	    if (yourHand.getHandValue()>21) {
	    	...
	    	currentMoney(win,lose);
		         ...
		         else text = "You have " + yourHand.getHandValue() + ".  Hit, Double down or stand?";
	    repaint();
	    }
	...
	private void pressSurrender(){
		if (gameStatus==false){text = "Click \"Play Again\" to play again.";
		repaint();return;}
		if (canSurrender==false) text="You can't surrender. Only in first move of the turn you can use this option.";
	...
	private void pressPlayAgain(){
		if(gameStatus==false) text="You have to finish this turn!";
		if (gameStatus==false){gameStatus=true;win=true;
	      lose=true;canSurrender=true;
	      newBet();
	      showCards();
		isBlackJack();
		}
	...
	private void pressDoubleDown(){
		if(yourMoney>=bet*2){bet=bet*2;
		pressHit();pressStand();
	}
		else JOptionPane.showMessageDialog(null, "You don't have enough money for double down"); 
