package com.gellert.digitalcatalog.database;
...
public class CatalogDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static CatalogDatabaseHelper instance = null;
    ...
    public static CatalogDatabaseHelper getInstance(Context context) {
        if(instance == null) {
            instance = new CatalogDatabaseHelper(context.getApplicationContext());
        }
        return instance;
    }
    ...
    public String getStudentClassName(int personID) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor result = db.rawQuery("SELECT CLASSES.NAME FROM CLASSES " +
                                    "JOIN STUDENT_CLASS ON (CLASSES.classID = STUDENT_CLASS.classID) " +
    ...
    public ArrayList<Mark> getStudentMarks(int personID) {
        ...
        Cursor result = db.rawQuery("SELECT MARKS.markID, SUBJECTS.NAME, MARKS.MARK, MARKS.MIDTERM FROM MARKS " +
                                    ...
                                    " ORDER BY SUBJECTS.NAME ASC, MARKS.MIDTERM DESC, MARKS.MARK DESC;", null);
        if (result == null || !result.moveToFirst()) return null;
        while (!result.isAfterLast()) {
            ...
            boolean midterm;
            midterm = result.getInt(result.getColumnIndex("MIDTERM")) != 0;
    ...
    public ArrayList<Mark> getStudentMarksForSubject(int personID, int subjectID) {
        ArrayList<Mark> marks = new ArrayList<Mark>();

        SQLiteDatabase db = this.getReadableDatabase();
        Cursor result = db.rawQuery("SELECT MARKS.markID, SUBJECTS.NAME, MARKS.MARK, MARKS.MIDTERM FROM MARKS " +
                "JOIN SUBJECTS ON (SUBJECTS.subjectID = MARKS.subjectID) " +
                "WHERE personID =" + personID + " AND SUBJECTS.subjectID = " + subjectID +
                " ORDER BY SUBJECTS.NAME ASC, MARKS.MIDTERM DESC, MARKS.MARK DESC;", null);
        if (result == null || !result.moveToFirst()) return null;
        while (!result.isAfterLast()) {
            int markID = result.getInt(result.getColumnIndex("markID"));
    ...
    public Student getStudentInfoFromDatabase(int personID) {
        String name = getPersonName(personID);
        String className = getStudentClassName(personID);
        ArrayList<Mark> marks = getStudentMarks(personID);

        return new Student(personID, name, className, marks);
    }

    public Student getStudentInfoFromDatabaseForSubject(int personID, int subjectID) {
        String name = getPersonName(personID);
        String className = getStudentClassName(personID);
        ArrayList<Mark> marks = getStudentMarksForSubject(personID, subjectID);
    ...
    public ArrayList<String> getTeacherSubjects(int personID) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor result = db.rawQuery("SELECT DISTINCT SUBJECTS.NAME FROM SUBJECTS " +
                "JOIN TEACHER_SUBJECT_CLASS ON (SUBJECTS.subjectID = TEACHER_SUBJECT_CLASS.subjectID) " +
                "WHERE personID = " + personID + ";", null);
        if (result == null || !result.moveToFirst()) return null;
    ...
    public Teacher getTeacherInfoFromDatabase(int personID) {
        String name = getPersonName(personID);
        ArrayList<String> subjects = getTeacherSubjects(personID);
    ...
    public ArrayList<String> getStudentNames(int classID) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor result = db.rawQuery("SELECT PERSONS.NAME FROM PERSONS " +
                                    "JOIN STUDENT_CLASS ON (PERSONS.personID = STUDENT_CLASS.personID) " +
    ...
    public int getPersonIDByName(String name) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor result = db.rawQuery("SELECT personID FROM PERSONS WHERE NAME = '" + name + "';", null);
        if (result == null || !result.moveToFirst()) return -1;
        return result.getInt(result.getColumnIndex("personID"));
    }
    ...
    public int getMaxMarkID() {
        ...
        Cursor result = db.rawQuery("SELECT MAX(markID) AS markID FROM MARKS;", null);
        if (result == null || !result.moveToFirst()) return -1;
        return result.getInt(result.getColumnIndex("markID"));
    }

    public void addMark(int personID, int subjectID, int mark, boolean midterm) {
        SQLiteDatabase db = this.getWritableDatabase();
        int markID = getMaxMarkID() + 1;
        int midtermValue;
        if(midterm) midtermValue = 1;
        else midtermValue = 0;

        ContentValues markValues = new ContentValues();
        markValues.put("markID", markID);
        markValues.put("personID", personID);
        markValues.put("subjectID", subjectID);
        markValues.put("MARK", mark);
        markValues.put("MIDTERM", midtermValue);

        db.insert("MARKS", null,markValues);
    }

    public void updateMark(int markID, int personID, int subjectID, int mark, boolean midterm) {
        SQLiteDatabase db = this.getWritableDatabase();
        String filter = "markID = " + markID;
        ...
        db.update("MARKS", markValues, filter, null);
    }

    public void deleteMark(int markID) {
        SQLiteDatabase db = this.getWritableDatabase();
        String filter = "markID = " + markID;
        db.delete("MARKS", filter, null);
    }
}
