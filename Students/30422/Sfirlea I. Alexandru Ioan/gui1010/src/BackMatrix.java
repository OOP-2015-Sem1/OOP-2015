...
public class BackMatrix {
	...
	/*
	 * calculam scorul si il returnam
	 * se ofera bonus pentru mai multe linii pline
	 * */
...
/*
 * verificam dacac putem muta piesa acolo
 * 
 * */
	...
	public  void pieceMoved(Piece randomPiece) {
		int temp[][] = new int[randomPiece.getDimI()][randomPiece.getDimJ()];
		for (int i = 0; i < randomPiece.getDimI(); i++) {
			for (int j = 0; j < randomPiece.getDimJ(); j++) {
				temp[i][j] = 0;
			}
