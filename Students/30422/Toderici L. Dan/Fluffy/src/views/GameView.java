...
public class GameView extends JFrame {
	...
	public GameView(char[][] gameMatrix, char[][] cluesMatrix, Fluffy fluffy) {
		...
		//this.add(new JButton("aaa"),BorderLayout.PAGE_START);
	...
	public void move(Fluffy fluffy, int xOffset, int yOffset) {
		...
		String nextPosition = gameLabelMatrix[fluffy.getFluffyX() + xOffset][fluffy.getFluffyY() + yOffset].getText();
		if(!nextPosition.equals("*"))
		{
			newSetup(xOffset, yOffset);
		}
	...
	public void newSetup(int xOffset, int yOffset) {
	...
	public void keepYellowClues() {
		...
		for (int i = 0; i < nbOfRows; i++) {
			for (int j = 0; j < nbOfCols; j++) {
				if ((!gameLabelMatrix[i][j].getText().equals(cluesMatrix[i][j])) && (cluesMatrix[i][j] == 'H')
						&& (!gameLabelMatrix[i][j].getText().equals("F"))) {
