...
public class Controller {
		...
		private DifficultyChooser colorGame = new DifficultyChooser();
		private DifficultyChooser memoryGame = new DifficultyChooser();
	...
	private class ChoiceListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent event) {
			if(event.getSource() == colorGameButton)
				colorGame.start(colorGameHeader,"color");
			else
				if(event.getSource() == memoryGameButton)
					memoryGame.start(memoryGameHeader,"memory");
			chooser.dispose();
		}
	}
}
